...
class FederatedLearningAggregator:
    def __init__(self):
        ...
        self.history = []
    def aggregate_models(self, local_models, add_log_callback):
        if not local_models:
        ...
        self.evaluate_global_model(add_log_callback)
        
        # 返回聚合结果用于UI展示
        return {"local_weights": [m.coef_[0].tolist() for m in local_models], "global_weights": avg_weight[0].tolist()}

    def evaluate_global_model(self, add_log_callback):
        X_test = np.array([[0.8, 0.1, 10], [0.2, 0.8, 22], [0.5, 0.5, 16]])
        y_test = np.array([2, 1, 0])
        X_test_scaled = self.scaler.transform(X_test)
        accuracy = self.global_model.score(X_test_scaled, y_test)
        self.history.append(accuracy)
        add_log_callback(f"[FL] 新全局模型准确率: {accuracy:.2f}", "fl")

# --- 3. 电动汽车 (EV) 和充电站模拟 (重大修改) ---
class ElectricVehicle:
    # ... (内部逻辑无变化, 只需传递 add_log_callback) ...
...
class ChargingStation:
    def __init__(self, num_evs=8, fl_interval=10):
        ...
        self.logs = deque(maxlen=15) # 新增：日志队列
        self.fl_round_details = {} # 新增：FL细节
    ...
    def simulation_step(self):
        ...
        self.fl_round_details = {} # 每一步都清空细节
        ...
        if countdown == 1: self.fl_status_text = "准备训练..."
            ...
            aggregation_result = self.fl_aggregator.aggregate_models(local_models, self.add_log)
            if aggregation_result:
                self.fl_round_details = {
                    "participants": self.fl_participants,
    ...
    def get_status(self):
        return {
            ...
            "logs": list(self.logs), # 新增
            "fl_round_details": self.fl_round_details # 新增
        }

# --- 4. Flask Web应用 (HTML/CSS/JS 再次大幅修改) ---
app = Flask(__name__)
...
<head>
    ...
    <style>
        ...
        .log-item .time { color: #999; } .log-item .cat-ev { color: #81d4fa; } .log-item .cat-blockchain { color: #a5d6a7; } .log-item .cat-fl { color: #ffd54f; }
...
<body>
    ...
    <script>
        ...
        function initChart() {
            ...
            flChart = new Chart(ctx, { type: 'line', data: { labels: [], datasets: [{ label: '全局模型准确率', data: [], borderColor: 'var(--primary-color)', backgroundColor: 'rgba(63, 81, 181, 0.1)', fill: true, tension: 0.2 }] }, options: { scales: { y: { beginAtZero: true, max: 1.0 } }, animation: { duration: 500 } } });
        }
        ...
        function updateUI(data) {
            ...
            if (isNewBlock) {
                ...
                blockchainContainer.innerHTML = ''; // Force full redraw on new block
                const newBlock = data.blockchain[data.blockchain.length - 1];
            }
            // Only redraw if necessary to avoid flicker
